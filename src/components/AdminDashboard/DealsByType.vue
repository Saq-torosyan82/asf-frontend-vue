<template>
  <div class="component hasRibbon" id="deals-by-type">
    <div class="ribbon"></div>
    <h5 class="component-header-text">Deals by Type</h5>
    ...
    <!-- <div class="legend">
      <div v-for="(deal, index) in deals.data" :key="index" class="line">
        <div
        ...
        ></div>
        <p>{{ deal.label }}</p>
        <p class="ammount">{{ deal.number }}</p>
      </div>
    </div> -->
    <div class="legend">
      <div class="line-wrap" v-for="(deal, index) in deals.data" :key="index">
        <template>
          <div
            class="line"
            :class="[
              seeOther ? 'visible' : '',
              index >= 2 && deals.data.length != 3 ? 'other-line' : '',
            ]"
          >
            <div
              class="color-indicator"
        ...
        <template
          v-if="deals.data.length > 3 && index + 1 == deals.data.length"
        >
...
export default {
  ...
  computed: {
    deals() {
      return store.getters['dashboard/getDeals'];
    },
    chartData() {
      let data = {
        labels: [],
      ...
      this.deals.data.map(function (item, key) {
    ...
    otherValue() {
      let sum = 0;
      this.deals.data.forEach((item, i) =>
        i >= 2 ? (sum += item.number) : ''
      );
      return sum;
    },
  ...
  methods: {
    // onDealsByType() {
    //   this.$router.push(`/admin/deals-by-type`);
    // },
    // toggleOther() {
